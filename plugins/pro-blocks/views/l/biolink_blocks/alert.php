...
<div id="<?= 'biolink_block_id_' . $data->link->biolink_block_id ?>" data-biolink-block-id="<?= $data->link->biolink_block_id ?>" class="col-12 my-2">
    <div class="alert <?= 'link-btn-' . $data->link->settings->border_radius ?> <?= $data->link->design->link_class ?>" style="<?= $data->link->design->link_style ?>" data-background-color data-border-width data-border-radius data-border-style data-border-color data-border-shadow data-text-alignment data-text-color>
        ...
        <button type="button" class="close" data-dismiss="alert" onclick="set_cookie('<?= 'dismiss_close_button_' . $data->link->biolink_block_id ?>', 1, <?= $data->link->settings->alert_pause_after_closed / 60 / 24 ?>, '<?= COOKIE_PATH ?>')"><i class="fa fa-fw fa-sm fa-times" style="<?= 'color: ' . $data->link->settings->text_color . '; opacity: .5;' ?>" data-text-color></i></button>
        <?php endif ?>
    </div>
</div>
<?php endif ?>
