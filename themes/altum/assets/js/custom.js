...
(function($) {
  var scrollstart = $(window).scrollTop();
    ...
    function load_dynamic_JS(url, implementationCode, location) {
    ...
    /* Document ready */
      ...
      /* Detection if visible on screen */
    ...
    /* Show Chat after 5s */
    setTimeout(() => {
        $('.crisp-client').addClass("visible");
    }, 5000); 
    // load slick
    if ($('.allinks-slider').length > 0) {
      load_dynamic_JS('/themes/altum/assets/js/slick@1.8.1.min.js', create_allinks_sliders, document.body);
      load_dynamic_CSS('/themes/altum/assets/css/slick@1.8.1.min.css');
    }
    /* *** */
    function create_allinks_sliders() {
        if ($('.slider-demos').length > 0) {
            $('.slider-demos').slick({
                slidesToShow: 5,
                slidesToScroll: 1,
                centerMode: true,
    ...
    /* FAQ */
    var acc = document.getElementsByClassName("toggle-faq");
    var i;
    for (i = 0; i < acc.length; i++) {
      acc[i].addEventListener("click", function() {
        this.classList.toggle("active");
        $( ".question" ).parent().removeClass('active');
        $( ".question.active" ).parent().addClass('active');
    ...
    var counter = 0 ;
    var uniqueclasseffect = null ;
    var anim = null ;
    var classcible = null ;
    var animready = false ;
    var elementtoanimate = null;
    /* Ajout d'une class unique pour chaque element de la page à animer */
    if ($('.anim').length > 0 ){
        addclassanim();
    }
    function addclassanim() {
    animready = true;
      if ($('.anim').length > 0){
          $(".anim").each(function(){
            uniqueclasseffect = 'counter' + counter;
            $(this).addClass(uniqueclasseffect);
            counter++;
            });
        }
    }
    /* Declenchement des effets */
    function fadeInUp(elementtoanimate,fadeindelay){
        classcible = elementtoanimate ;
        $(classcible).addClass('animated fadeInUp');
        if(fadeindelay){
            $(classcible).css('animation-delay',fadeindelay);
        }
        return this;
    }
    /* Si élément déjà présent sur la page sans scroll, on l'anime */
    if (animready){
        var numeranimstart = 0 ;
        var fadeindelay1 = 0;
        $(".anim").each(function(){
            if ($('.counter' + numeranimstart).length > 0 && $('.counter' + numeranimstart).isVisible()) {
                elementtoanimate = '.counter' + numeranimstart;
                if ($('.fadetop.delay.counter' + numeranimstart).length > 0) {
                    if($('.counter' + numeranimstart).data("delay") !== undefined){
                        fadeInUp(elementtoanimate,$('.counter' + numeranimstart).data("delay")+'s');
                    } else {
                        fadeInUp(elementtoanimate,fadeindelay1+'s');
                    }
                    fadeindelay1 += 0.2;
                } else {
                    fadeInUp(elementtoanimate);
                }
            }
        numeranimstart++;
        })
    ...
    $(window).on('scroll', throttle(function (event) {
            ...
            if (animready){
                $(".anim").each(function(){
                    if ($('.counter' + numeranim).length > 0 && $('.counter' + numeranim).isVisible()) {
                        elementtoanimate = '.counter' + numeranim;
                        if ($('.fadetop.counter' + numeranim).length > 0 && $('.fadetop.counter' + numeranim).isVisible()) {
                            if ($('.fadetop.delay.counter' + numeranim).length > 0) {
                                if($('.counter' + numeranim).data("delay") !== undefined){
                                    fadeInUp(elementtoanimate,$('.counter' + numeranim).data("delay")+'s');
                                } else {
                                    fadeInUp(elementtoanimate,fadeindelay+'s');
                                }
                                fadeindelay += 0.2;
                            } else {
                                fadeInUp(elementtoanimate);
                        ...
                        if ($('.slideup.counter' + numeranim).length > 0 && $('.slideup.counter' + numeranim).isVisible()) {
                            slideUp(elementtoanimate);
                        }
                        if ($('.fade.counter' + numeranim).length > 0 && $('.fade.counter' + numeranim).isVisible()) {
                            fadeIn(elementtoanimate);
                        }
                        if ($('.slideleft.counter' + numeranim).length > 0 && $('.slideleft.counter' + numeranim).isVisible()) {
                          slideLeft(elementtoanimate);
                        }
                        if ($('.slideright.counter' + numeranim).length > 0 && $('.slideright.counter' + numeranim).isVisible()) {
                          slideRight(elementtoanimate);
                        }
                    }
                  /* UNLOAD ANIM & vidéo IF NOT ON SCREEN*/
                  // if ($('.counter' + numeranim).length > 0 && !($('.counter' + numeranim).isVisible())) {
                  //   restartAnim('.counter' + numeranim);
                  // }  
                  /* *** */
                numeranim++;
                })
