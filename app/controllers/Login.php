...
class Login extends Controller {
    ...
    public function index() {
        ...
        if (settings()->apple->is_enabled && in_array($method, ['apple-initiate', 'apple'])) {
            ...
            $redirect_uri = (SITE_URL . 'login/apple');
            $_SESSION['state'] = bin2hex(random_bytes(5));
            $keyId = settings()->apple->keyId;
            $teamId = settings()->apple->teamId;
            $privateKeyFilePath = (SITE_URL . 'app/config/apple/AuthKey_GG88V296BA.p8');
            $algorithm = 'ES256';
...
//                return (new Response())->withHeader('Location', $authorize_url)->withStatus(302);
            ...
            if ($method == 'apple' && isset($_POST['code'])) {
                ...
                // Convert the .p8 private key into PEM format
                $privateKeyContents = file_get_contents($privateKeyFilePath);
                $privateKey = openssl_pkey_get_private($privateKeyContents);

                $secret_key = <<<EOD
-----BEGIN PRIVATE KEY-----
MIGTAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBHkwdwIBAQQgk33Agfoxx5sm54lp
oGzgf1yv3U3dtKzelhTsyQ395TegCgYIKoZIzj0DAQehRANCAAQBi2ovfrscBmA4
NFiAs3hZiUNG+I3EjMso3QgyfQe/frkUPkPCm93RUFNy8o9LFxD85RWZ5F29tH32
WbBhTT76
-----END PRIVATE KEY-----
EOD;
                ...
                $clientSecret = JWT::encode($payload, file_get_contents($privateKeyFilePath), $algorithm, $keyId);
...
//                        echo "<script>alert('" . curl_error($ch) . "');</script>";
                        ...
                        // User's access token received from the previous step
                        $access_token = $data['access_token'];
// Step 5: Use access token to fetch user details

                        $id_token = $data['id_token'];

                        // Verify and decode the ID token
                        $decoded_token = base64_decode(str_replace(array('-', '_'), array('+', '/'), explode('.', $id_token)[1]));
                        $token_data = json_decode($decoded_token, true);

                        // Extract user details
                        $user_id = $token_data['sub']; // Unique user identifier
                        $email = $token_data['email'] ?? '';
                        $name = $token_data['name'] ?? '';

                        $this->process_social_login($email, $user_id, $redirect, $method);
                    }
                }
            }
        }
