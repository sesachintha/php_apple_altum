...
class BiolinkBlockAjax extends Controller {
    ...
    public function duplicate() {
        ...
        /* Get the link data */
        $biolink_block = db()->where('biolink_block_id', $_POST['biolink_block_id'])
            ->where('user_id', $this->user->user_id)->getOne('biolinks_blocks');
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $new_block_order = $biolink_block->order + 1;
            /* Database query */
            $new_block_id = db()->insert('biolinks_blocks', [
            ...
            // Check if the new biolink's order conflicts with any existing biolinks
            $conflicting_biolinks = db()->where('`order`', $new_block_order, '>=')
                ->where('`biolink_block_id`', $new_block_id, '!=')
                ->where('user_id', $this->user->user_id)->get('biolinks_blocks');
//            echo $new_block_order;
//            echo "<pre>" . print_r($conflicting_biolinks) . '</pre>';
//            echo "<pre>" . print_r(db()->where('user_id', $this->user->user_id)->get('biolinks_blocks')) . '</pre>';
            if (count($conflicting_biolinks)) {
                foreach ($conflicting_biolinks as $conflicting_biolink){
                    // Increment the new biolink's order and update the database
                    db()->where('biolink_block_id', $conflicting_biolink->biolink_block_id)
                        ->where('user_id', $this->user->user_id)
                        ->update(
                            'biolinks_blocks', [
                                'order' => $conflicting_biolink->order + 1
                            ]
                        );
                }
            }
//            echo "\nNEXT\n";
//            echo "<pre>" . print_r(db()->where('user_id', $this->user->user_id)->get('biolinks_blocks'));die;
            /* Clear the cache */
            \Altum\Cache::$adapter->deleteItem('link?link_id=' . $biolink_block->link_id);
    ...
    private function create() {
        ...
        if(isset($_POST['block_type']) && array_key_exists($_POST['block_type'], $this->biolink_blocks)) {
            ...
            $individual_blocks = ['link', 'heading', 'paragraph', 'avatar', 'socials', 'email_collector', 'rss_feed', 'custom_html', 'vcard', 'image', 'image_grid', 'divider', 'list', 'alert', 'faq', 'timeline', 'review', 'image_slider', 'discord', 'countdown', 'cta', 'external_item', 'share', 'youtube_feed', 'paypal', 'phone_collector', 'donation', 'product', 'service', 'map', 'opensea'];
            $embeddable_blocks = ['anchor', 'applemusic', 'soundcloud', 'spotify', 'tidal', 'tiktok_video', 'twitch', 'twitter_tweet', 'vimeo', 'youtube', 'instagram_media', 'facebook', 'reddit'];
    ...
    private function update_biolink_link() {
        ...
        $_POST['border_shadow_color'] = !preg_match('/#([A-Fa-f0-9]{3,4}){1,2}\b/i', $_POST['border_shadow_color']) ? '#000' : $_POST['border_shadow_color'];
    ...
    private function update_biolink_avatar() {
        ...
        $_POST['border_shadow_spread'] = in_array($_POST['border_shadow_spread'], range(0, 10)) ? (int) $_POST['border_shadow_spread'] : 0;
        $_POST['border_shadow_color'] = !preg_match('/#([A-Fa-f0-9]{3,4}){1,2}\b/i', $_POST['border_shadow_color']) ? '#000' : $_POST['border_shadow_color'];
        $_POST['object_fit'] = in_array($_POST['object_fit'], ['contain', 'cover', 'fill']) ? query_clean($_POST['object_fit']) : 'cover';
